# vibe.fm — Setup Playbook (Hackathon Sprint)
**Status:** Minimal. No Conda, no Essentia, no audio processing libs.
...
```bash
npx expo install expo-haptics expo-location expo-file-system react-native-reanimated
npx expo install @siteed/expo-audio-studio moti
npm install react-native-gesture-handler
```

Prebuild (required for audio + haptics native modules):
```bash
...
EXPO_PUBLIC_FREQBLOG_API_KEY=your_key_here
EXPO_PUBLIC_OPENROUTER_API_KEY=your_key_here
```

Access in app: `process.env.EXPO_PUBLIC_*` (Expo managed workflow standard).

---

## 3. Pre-batch Script (Run Once Before Demo)

**File:** `seed.py` — no audio, no ffmpeg, just HTTP requests.
...
```
Outputs: `mock_tracks.json`, `mock_profiles.json`, `sonic_profiles.json` in current dir.
Copy all three into the Expo app's `assets/data/` folder.

---

## 4. Critical Technical Caveats

### Shazam — HTTP 204 avoidance
- Live mic: `@siteed/expo-audio-studio` `onAudioStream` emits raw PCM base64. No header. Send directly.
- File upload: decode base64 → strip first 44 bytes → re-encode → send. Never strip base64 characters directly (breaks byte alignment).
- Audio requirements: 44100Hz, mono, signed 16-bit PCM LE, raw bytes only, < 500KB.

### iOS Haptic + Recording conflict
- Taptic engine disabled during active recording session.
- Poll with `setInterval` until hardware status = `disconnected`, then trigger `Haptics.impactAsync`.
- Never rely on `stopRecording()` promise alone — OS may still hold the lock.

### Gemini JSON enforcement
- Always set `response_format: { type: "json_object" }` + `reasoning: { exclude: true }`.
- Always wrap Gemini call in try/catch. If it fails, show fingerprint bars without description — never block the result sheet.

### FreqBlog failure
- If 4xx/5xx or timeout: show "Analysis unavailable" where description would be. Still show Shazam result (title/artist). Never crash.
